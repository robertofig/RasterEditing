...
int main(int Argc, char** Argv)
{
    if (Argc > 1)
    {
        ...
        poly_info Poly = RasterToOutline(Argv[1]);
        ...
        {
            ring_info* Ring = Poly.Rings[RingIdx];
            shp_ring_type Type = (Ring->Type == 0) ? ShpRing_Outer : ShpRing_Inner;
            AddRing(&Feat, Type, Ring->NumVertices, Ring->Vertices, 0, 0, sizeof(v2));
        }
#else
        shapefile Out = CreatePolyline(OutShp, OutShx, OutDbf);
        shp_feature Feat = AddFeature(&Out, 1, Poly.NumVertices);
        AddLinestring(&Feat, Poly.NumVertices, Poly.Vertices, 0, 0, sizeof(v2));
#endif
